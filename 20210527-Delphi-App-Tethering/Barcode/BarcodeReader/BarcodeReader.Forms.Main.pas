...
type
  TMainForm = class(TForm)
    ...
    procedure CodeLabelClick(Sender: TObject);
  private
    FMessageSubscriptionID: Integer;
    procedure HandleActivityMessage(const Sender: TObject; const M: TMessage);
    function HandleActivityResult(RequestCode, ResultCode: Integer;
...
procedure TMainForm.CodeLabelClick(Sender: TObject);
begin
  // Send the last captured code to connected apps.
  SendBarcodeValue(CodeLabel.Text);
end;
...
begin
  ...
  if M is TMessageResultNotification then
    HandleActivityResult(TMessageResultNotification(M).RequestCode,
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LScanContent := JStringToString
          (Data.getStringExtra(StringToJString('SCAN_RESULT')));
        LScanFormat := JStringToString
